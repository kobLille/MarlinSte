...
void setup()
{
  ...
  #endif
  fanSpeed = 200;
  SERIAL_ECHOLN("fin setup");
  SERIAL_ECHOLN("");
  
   //info divers test
  SERIAL_ECHO("info lit chauffant :");
//  SERIAL_ECHOLN(current_temperature_bed);

//SERIAL_ECHO(updateTemperaturesFromRawValues());
//SERIAL_ECHO(", ");
SERIAL_ECHO(degHotend(0));
SERIAL_ECHO(", ");
SERIAL_ECHO(degHotend(1));
SERIAL_ECHO(", ");
  SERIAL_ECHOLN(degBed());
    SERIAL_ECHOLN("");
}
      ...
      target_direction = isHeatingHotend(tmp_extruder); // true if heating, false if cooling
        ...
        while ( target_direction ? (isHeatingHotend(tmp_extruder)) : (isCoolingHotend(tmp_extruder)&&(CooldownNoWait==false)) ) {
          ...
          { //Print Temp Reading and remaining time every 1 second while heating up/cooling down
            SERIAL_PROTOCOLPGM("M109 T:");
            SERIAL_PROTOCOL_F(degHotend(tmp_extruder),1);
            SERIAL_PROTOCOLPGM(" E:");
            SERIAL_PROTOCOL((int)tmp_extruder);
            
            SERIAL_ECHO("Marlin_main.cpp manage info lit chauffant :");
...
SERIAL_ECHOLN(current_temperature_bed);
    ...
    case 190: // M190 - Wait for bed heater to reach target.
      ...
      //  while ( target_temperature_bed-5 < (int)current_temperature_bed)
      //  {
          if(( millis() - codenum) > 1000 ) //Print Temp Reading every 1 second while heating up.
          {
            float tt=degHotend(active_extruder);
            SERIAL_PROTOCOLPGM("M190 T:");
            SERIAL_PROTOCOL(tt);
            SERIAL_PROTOCOLPGM(" E:");
            ...
            SERIAL_PROTOCOL(isHeatingBed());
            SERIAL_PROTOCOLLN("");
            
               SERIAL_PROTOCOL(" M190 suite:");
            SERIAL_PROTOCOL(target_temperature_bed);
            SERIAL_PROTOCOL(" : ");
